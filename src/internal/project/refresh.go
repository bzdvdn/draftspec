...
const (
	agentsBlockStart = "<!-- draftspec:start -->"
	agentsBlockEnd   = "<!-- draftspec:end -->"
)
...
func Refresh(root string, options RefreshOptions) (RefreshResult, error) {
	...
	for _, file := range templateFiles {
		if file.TargetPath == "draftspec.yaml" || file.TargetPath == "constitution.md" {
			continue
		}
...
func upsertManagedAgentsBlock(current, block string) string {
	...
	if legacy := strings.Index(current, "## Draftspec"); legacy >= 0 {
