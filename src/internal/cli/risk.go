...
Examples:
  draftspec risk my-feature
  draftspec risk my-feature --json
`,
...
func assessDataChanges(root, slug string) RiskFactor {
	plansDir := filepath.Join(root, ".draftspec", "plans")
	dataModelPath := filepath.Join(plansDir, slug, "data-model.md")
...
func assessAPIChanges(root, slug string) RiskFactor {
	plansDir := filepath.Join(root, ".draftspec", "plans")
	contractsDir := filepath.Join(plansDir, slug, "contracts")
