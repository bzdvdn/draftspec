...
func newInternalCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:           "__internal",
		Short:         "Internal Draftspec helpers",
		Hidden:        true,
		SilenceUsage:  true,
		SilenceErrors: true,
	}
...
func newInternalCheckSpecReadyCmd() *cobra.Command {
	var root string
	cmd := &cobra.Command{
		Use:           "check-spec-ready",
		Hidden:        true,
		SilenceUsage:  true,
		SilenceErrors: true,
		RunE: func(cmd *cobra.Command, args []string) error {
			result, err := workflow.CheckSpecReady(root)
			return renderCheckResult(cmd, result, err)
		},
	}
	cmd.Flags().StringVar(&root, "root", ".", "Draftspec project root")
...
func newInternalCheckArchiveReadyCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			result, err := workflow.CheckArchiveReady(root, args[0], args[1], args[2])
			return renderCheckResult(cmd, result, err)
		},
	}
	cmd.Flags().StringVar(&root, "root", ".", "Draftspec project root")
