...
Includes phase, task progress, readiness, and branch mismatch hints.`,
		Example: "  draftspec dashboard .\n  draftspec dashboard . --all\n  draftspec dashboard . --archived\n  draftspec dashboard . --json\n  draftspec dashboard /path/to/project",
			...
			if len(displayed) == 0 {
				if len(active) == 0 && len(archived) > 0 && !includeArchived && !archivedOnly {
					printPanel(w, "No Active Features Found", []string{
						"Use " + styleCmd(w, "draftspec dashboard . --archived") + " to view archived features.",
						"Or use " + styleCmd(w, "draftspec dashboard . --all") + " to view everything.",
					})
					return nil
				}
				printPanel(w, "No Features Found", []string{
					"Create your first spec with " + styleCmd(w, "/draftspec.spec") + ".",
					"Or run " + styleCmd(w, "draftspec demo") + " to explore an example workspace.",
				})
				return nil
			}
...
func printDashboardSummary(w io.Writer, summary dashboardSummary) {
	...
	printPanel(w, "draftspec dashboard", []string{
...
func printDashboardLegend(w io.Writer) {
	...
	printPanel(w, "Legend", []string{
		...
		"progress bar uses task checklist counts from plans/<slug>/tasks.md",
	})
}
