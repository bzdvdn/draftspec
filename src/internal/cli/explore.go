...
Examples:
  draftspec explore .
  draftspec explore . --name "Investigate auth performance"
  draftspec explore . --slug "auth-perf" --name "Auth performance investigation"
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintf(cmd.OutOrStdout(), "  1. Use /draftspec.explore %s to investigate\n", slug)
			fmt.Fprintf(cmd.OutOrStdout(), "  2. When ready, call /draftspec.spec %s to write the spec\n", slug)
			fmt.Fprintf(cmd.OutOrStdout(), "  3. Or call /draftspec.spec %s --from-explore to convert exploration to spec\n", slug)
...
- [ ] Готов перейти к /draftspec.spec

## Связанные артефакты

- Spec: specs/%s/spec.md (не создан)
- Plan: plans/%s/plan.md (не создан)
...
- [ ] Ready to proceed to /draftspec.spec

## Related Artifacts

- Spec: specs/%s/spec.md (not created)
- Plan: plans/%s/plan.md (not created)
`, name, slug, time.Now().Format("2006-01-02"), slug, slug)
}
