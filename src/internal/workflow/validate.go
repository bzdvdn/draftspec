...
func ValidateProject(root string) ([]Finding, error) {
	...
	for _, state := range states {
		...
		if state.InspectLegacy {
			findings = append(findings, Finding{Level: "warning", Message: fmt.Sprintf("legacy inspect report path in use for slug %s; run draftspec feature repair %s or draftspec migrate", state.Slug, state.Slug)})
		}
