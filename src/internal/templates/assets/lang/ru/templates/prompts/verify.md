...
Подтвердите, что реализованная работа достаточно согласована с задачами, памятью проекта и правилами проекта, чтобы безопасно двигаться дальше.
...
- Проверяйте, что `.draftspec/memory.md` отражает важные изменения реализации, когда это уместно.
...
- Используйте `blocked`, если отсутствие завершенных задач, противоречивое состояние реализации или несинхронизированная `memory.md` делают архивирование или заявление о завершенности небезопасным.
...
- Используйте `.draftspec/scripts/verify-memory-sync.sh <slug>`, чтобы до более глубокой проверки поймать грубые противоречия между `tasks.md` и `memory.md`.
...
- В `## Checks` явно отразите завершенность задач, грубую memory-sync проверку, синхронизацию `memory.md` и согласованность реализации в тех местах, которые реально проверялись
