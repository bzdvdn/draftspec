...
Когда `--delta` присутствует в `$ARGUMENTS`:
- Сначала прочитайте существующий `.draftspec/specs/<slug>/inspect.md` для установления baseline.
...
- Если изменения слишком обширны (перезаписано более 50% спека), откатитесь к полной проверке с пометкой: «Delta mode откатился к полной проверке из-за масштаба изменений.»
