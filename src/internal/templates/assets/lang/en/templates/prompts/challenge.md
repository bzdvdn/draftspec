...
Write `.draftspec/plans/<slug>/challenge.md` using this structure:
...
- Write the file to `.draftspec/plans/<slug>/challenge.md`.
- Also output a compact inline summary: verdict, top 1–3 findings, recommended action.
- End the conversation with the exact next step (e.g. refine a specific AC, create `research.md`, or `/draftspec.implement <slug>` if the feature is strong).
