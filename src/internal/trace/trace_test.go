...
func TestTracePattern(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			line:     "// @ds-task T1.1: Add something (AC-001)",
			wantType: "task",
			wantTask: "T1.1",
			wantAC:   "AC-001",
		...
		},
		{
			...
			line:     "// @ds-test T1.1: TestSomething (AC-002)",
			wantType: "test",
			wantTask: "T1.1",
			wantAC:   "AC-002",
		...
		},
		{
			...
			line:     "// @ds-task T2.2: Simple description",
			wantType: "task",
			wantTask: "T2.2",
			wantAC:   "",
		...
		},
		{
			...
			line:     "/* @ds-task T4.4: In block comment (AC-004) */",
			wantType: "task",
			wantTask: "T4.4",
			wantAC:   "AC-004",
		...
		},
		{
			...
			line:     "# @ds-task T5.5: In shell script (AC-005)",
			wantType: "task",
			wantTask: "T5.5",
			wantAC:   "AC-005",
...
// @ds-task T1.1: Implementation (AC-001)
func Do() {}

// @ds-test T1.1: TestDo (AC-001)
func TestDo() {}
`
	...
	if err := os.WriteFile(skipFile, []byte("// @ds-task T9.9: Should skip"), 0o644); err != nil {
		t.Fatal(err)
	}
