...
- `draftspec check <slug>` показывает наличие артефактов, вердикт inspect и verify, прогресс задач и точную следующую slash-команду; выходит с кодом 1, если заблокировано; поддерживает `--json` для CI. `--all` выводит таблицу готовности по всем фичам.
