...
- `/draftspec.challenge`: adversarial review of spec or plan — finds weak assumptions, untestable AC, scope drift
- `/draftspec.handoff`: compact session handoff document for new sessions
- `/draftspec.hotfix`: emergency fix outside standard chain (≤3 files, known root cause)
- `/draftspec.scope`: quick scope boundary check (inline only, no file)
- `/draftspec.recap`: project-level overview of all active features

## Language model

`draftspec init` supports compact language configuration:
...
Language settings stored in `.draftspec/draftspec.yaml`.
...
- persisted inspect report at `.draftspec/specs/<slug>/inspect.md`
...
- use `draftspec trace` as primary evidence
...
`draftspec trace <slug> [path]` scans for `@ds-task` and `@ds-test` annotations in code.
...
Generated workspaces include launcher script under `.draftspec/scripts/`:
...
- `.draftspec/constitution.md`
- `.draftspec/specs/<slug>/spec.md`
- `.draftspec/specs/<slug>/inspect.md` (required)
...
`draftspec check --all` shows readiness table across all features.

`draftspec dashboard` provides visual dashboard of all active features.
...
- migrate legacy flat spec artifacts to `.draftspec/specs/<slug>/`
- migrate legacy inspect reports to `.draftspec/specs/<slug>/inspect.md`
...
- orphaned `@ds-task` annotations (warning)
