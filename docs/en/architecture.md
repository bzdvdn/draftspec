...
This page explains how Draftspec is structured internally.

## High-Level Layers

Draftspec is split into a few practical layers:

- `src/cmd/draftspec/` for the CLI entrypoint
...
- `src/internal/config/` for loading, defaulting, and saving `.draftspec/draftspec.yaml`
...
The config source of truth lives in `.draftspec/draftspec.yaml`.
...
This is what allows the rest of the codebase to avoid hardcoding too many `.draftspec/...` paths.

## Template and Asset Layer

Draftspec generates files from localized assets stored under:
...
The `templates` package assembles these assets into the generated `.draftspec/` workspace.
...
These generated files are wrappers that point back to canonical Draftspec prompts inside `.draftspec/templates/prompts/`.
...
- Does it push Draftspec closer to spec-kit-style bureaucracy without matching value?
