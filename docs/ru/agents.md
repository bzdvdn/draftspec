...
- перед записью `.draftspec/specs/<slug>.md` он должен создавать или переключать `feature/<slug>`, когда окружение это позволяет
- он должен поддерживать `--name`, optional `--slug` и optional `--branch` для chat-oriented ввода
- если `/draftspec.spec` вызван с `--name`, но без достаточного описания, он должен сохранить контекст и запросить или принять следующее сообщение как продолжение spec-запроса
