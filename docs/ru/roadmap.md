...
Ближайшая задача Draftspec не в том, чтобы догнать более тяжелые SDD-системы по количеству фаз, артефактов или автоматизации.
...
До первых полевых проверок Draftspec должен предпочитать:
...
До первых полевых проверок Draftspec не должен спешить с:

- новыми обязательными фазами
- обязательными persisted reports для каждой фичи
...
Release filter: `verify` должен оставаться легкой опциональной страховкой, а не новой тяжелой обязательной фазой для каждой фичи.
