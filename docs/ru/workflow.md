...
Draftspec должен предпочитать cheap helper findings как первый слой доказательств для inspect:
...
Draftspec использует **Ленивую декомпозицию** для экономии контекста:
...
`--repair <task-id-list>`: режим точечного исправления. Исправляет конкретные задачи, выявленные verify или review (напр. `--repair T2.3,T3.1`), не переписывая весь task list. Если repair выявляет проблему в плане — предлагает `/draftspec.plan --update`.
...
Формат: `// @ds-task <TASK_ID>: <Описание> (<AC_ID>)`
...
Проверка использует **данные прослеживаемости**, собранные с помощью `/.draftspec/scripts/trace.* <slug>`, чтобы подтвердить соответствие реализации заявленным задачам и критериям приемки (включая аннотации `@ds-test`).
...
`--persist`: записать отчёт в `.draftspec/plans/<slug>/verify.md` в дополнение к выводу в чат. Без этого флага отчёт остаётся только в чате.
