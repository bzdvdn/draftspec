...
- аннотировать каждое нетривиальное изменение кода меткой `// @ds-task <ID> (<AC_ID>)`
...
```go
// @ds-task T1.1: Добавить модель расписания партнера (AC-001)
...
- использовать `/.draftspec/scripts/trace.sh partner-scheduling` для сбора доказательств реализации
- искать аннотации `// @ds-task` и `// @ds-test` в коде
