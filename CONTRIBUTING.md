...
Thanks for your interest in improving `draftspec`.
...
- `/.draftspec/` is generated test output
...
If your change affects generated templates, scripts, prompts, or agent files, also verify the generated output path that `draftspec init` or `draftspec refresh` would produce.

## Workflow Expectations

- Keep changes focused and reviewable.
- Prefer small pull requests over broad mixed refactors.
- Preserve Draftspec's lightweight design goals unless there is a strong reason to widen the workflow.
- Keep generated assets, docs, prompts, config, and implementation aligned when behavior changes.
- Do not commit generated `/.draftspec/` or repository-local `AGENTS.md` test artifacts from self-hosting runs.
