...
function Resolve-LatestTag {
  ...
  $headers = @{ "User-Agent" = "draftspec-install" }
...
if (-not $BinDir -or $BinDir.Trim().Length -eq 0) {
  if ($env:DRAFTSPEC_INSTALL_DIR) {
    $BinDir = $env:DRAFTSPEC_INSTALL_DIR
  } else {
    $BinDir = Join-Path $env:LOCALAPPDATA "Programs\\draftspec\\bin"
  }
}

if (-not $AddToPath -and $env:DRAFTSPEC_ADD_TO_PATH) {
  $v = [string]$env:DRAFTSPEC_ADD_TO_PATH
  if ($v -match "^(1|true|yes|y|on)$") { $AddToPath = $true }
}

if ($Version -eq "latest" -and $env:DRAFTSPEC_VERSION) {
...
$arch = if ($env:DRAFTSPEC_ARCH) { $env:DRAFTSPEC_ARCH } else { Detect-Arch }
$asset = "draftspec_{0}_windows_{1}.zip" -f $Version, $arch
$url = "https://github.com/$RepoOwner/$RepoName/releases/download/$Version/$asset"

$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("draftspec-install-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  $exe = Join-Path $extractDir "draftspec.exe"
  if (-not (Test-Path $exe)) { Fail "Archive did not contain expected draftspec.exe" }

  New-Item -ItemType Directory -Path $BinDir -Force | Out-Null
  Copy-Item -Path $exe -Destination (Join-Path $BinDir "draftspec.exe") -Force

  Write-Host ("installed: {0}" -f (Join-Path $BinDir "draftspec.exe"))
  ...
  if ($AddToPath) {
    ...
    if (-not ($env:PATH -split ";" | Where-Object { $_ -eq $BinDir })) {
      ...
      Write-Host "note: rerun with -AddToPath (or set DRAFTSPEC_ADD_TO_PATH=1) to update PATH"
    }
  }

  & (Join-Path $BinDir "draftspec.exe") --version 2>$null
} finally {
