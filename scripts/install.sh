...
usage() {
  cat <<'EOF'
Install draftspec from GitHub Releases (Linux only).
...
add_to_path_if_needed() {
  ...
  printf '\n# Added by draftspec installer\n%s\n' "$export_line" >>"$profile_file"
  echo "updated PATH in: ${profile_file}"
}

main() {
  local version="latest"
  local bin_dir="${DRAFTSPEC_INSTALL_DIR:-$HOME/.local/bin}"
  local add_to_path="0"
  ...
  local arch
  arch="${DRAFTSPEC_ARCH:-$(detect_arch)}"

  if [[ "$version" == "latest" && -n "${DRAFTSPEC_VERSION:-}" ]]; then
    version="$DRAFTSPEC_VERSION"
  fi

  if [[ "$add_to_path" != "1" ]] && truthy "${DRAFTSPEC_ADD_TO_PATH:-}"; then
    add_to_path="1"
  fi
  ...
  local asset="draftspec_${version}_linux_${arch}.tar.gz"
  ...
  [[ -f "${tmpdir}/draftspec" ]] || fail "archive did not contain expected 'draftspec' binary"

  mkdir -p "$bin_dir"
  install -m 0755 "${tmpdir}/draftspec" "${bin_dir}/draftspec"

  echo "installed: ${bin_dir}/draftspec"
  ...
  if [[ "$add_to_path" == "1" ]]; then
    ...
    if ! command -v draftspec >/dev/null 2>&1; then
      ...
      echo "note: rerun with --add-to-path (or set DRAFTSPEC_ADD_TO_PATH=1) to update PATH"
    fi
  fi

  "${bin_dir}/draftspec" --version 2>/dev/null || true
}
