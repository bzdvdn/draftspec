...
This repository builds the `draftspec` CLI itself, so it is a natural dogfooding example.

## Why This Matters

Self-hosting is one of the strongest trust signals for Draftspec.
...
Generated self-hosting files such as `/.draftspec/` and repository-local `AGENTS.md` are test artifacts and should not be committed as canonical design inputs.
...
because that is where Draftspec's strictness is easiest to demonstrate.
